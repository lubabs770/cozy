#!/usr/bin/env bash
#
# cozy — standalone installer for plain Hyprland (cozy owns the wallpaper).
#
# Builds cozy and wires it into vanilla Hyprland: cozy renders the wallpaper
# itself (opaque mode) and stands in for a wallpaper daemon — do NOT also run
# hyprpaper/swww. (To run cozy *alongside* a daemon instead, use the
# swww-overlay integration once it ships.)
#
...
# Or run directly from a checkout:  integrations/standalone/install.sh
#
...
install -m 0755 "$SRC_DIR/integrations/standalone/cozy-session" "$BIN_DIR/cozy-session"
install -m 0755 "$SRC_DIR/integrations/standalone/cozy-wall"    "$BIN_DIR/cozy-wall"
...
# Rain/snow effect: droplet | ripple | snow | clouds
effect="droplet"
...
if [ "$choice" = "preshipped" ]; then
    cat > "$COZY_HYPR" <<EOF
# cozy — Hyprland integration (managed by the cozy installer; safe to edit).
...
# Switch the rain effect live (droplet | ripple | snow | clouds):
...
if [ ! -f "$HYPR_CONF" ]; then
    ...
    printf '\n# cozy wallpaper engine (added by the cozy installer)\n%s\n' "$SOURCE_LINE" >> "$HYPR_CONF"
    ok "Appended source line (backup: $(basename "$HYPR_CONF").cozy-bak)"
fi

# --- done ------------------------------------------------------------------
echo
printf '%scozy is installed for Hyprland (standalone).%s\n' "$GRN$BOLD" "$RST"
